...
public class Game_View extends JPanel implements ActionListener, KeyListener{
	// 게임의 주요 이미지 및 상태 변수
    ...
    private boolean ultimate = false;  // 궁극기 사용 추적 
    ...
    private Socket socket;	// 네트워크 소켓
    private PrintWriter out;  // 서버로 데이터 전송
    private BufferedReader in;  // 서버로부터 데이터 수신
    ...
    public Game_View(){
    	...
    	myFlightImage = new ImageIcon("./image/F4K.png").getImage();
    	otherFlightImage = new ImageIcon("./image/F35K.png").getImage();
        ...
        bexplosionImage = new ImageIcon("./image/Bexplosion.png").getImage();  // 비행기 최종 폭발하는 이미지
    	...
    	this.myX = 700;
    	this.myY = 200;
    	this.myFlipped = true; // 왼쪽을 향하도록 설정
    	
    	// 상대 좌표
    	this.otherX = 100;
    	this.otherY = 200;
    	this.otherFlipped = false; // 오른쪽을 향하도록 설정
    	
    	// 내 총알에 대한 상태 array(좌 -> 우 발사)
    	myBullets = new ArrayList<>();
    	// 내 총알에 대한 상태 array(우 -> 좌 발사)
		myBulletsFlipped = new ArrayList<>();
    	// 상대 총알에 대한 상태 array(좌 -> 우 발사)
    	otherBullets = new ArrayList<>();
    	// 상대 총알에 대한 상태 array(우 -> 좌 발사)    	
    	otherBulletsFlipped = new ArrayList<>();
    	
    	// 스페이스바를 계속 누르고 있을 때 총알이 연사로 나가는걸 방지(꼭 클릭 해야만 한 발씩 나감).
    	isBulletFired = false;
    ...
    private void updatePlayerPosition() {
    	if (myKeys[KeyEvent.VK_UP]) {
            myY -= PlayerSPEED; // 왼쪽 키가 눌리면 왼쪽으로 이동
        }
        if (myKeys[KeyEvent.VK_DOWN]) {
            myY += PlayerSPEED; // 오른쪽 키가 눌리면 오른쪽으로 이동
        }
        if (myKeys[KeyEvent.VK_LEFT]) {
            myX -= PlayerSPEED; // 위쪽 키가 눌리면 위로 이동
            myFlipped = true;
        }
        if (myKeys[KeyEvent.VK_RIGHT]) {
            myX += PlayerSPEED; // 아래쪽 키가 눌리면 아래로 이동
            myFlipped = false;
        }
    ...
    public void keyPressed(KeyEvent e) {
    	...
    	if (e.getKeyCode() == KeyEvent.VK_SPACE && !isBulletFired) {  
    		...
    		if (!myFlipped) {  // 좌 -> 우로 총알 발사 순간의 위치
    ...
    private void updateMyBullets() {  // 본인 총알 업데이트
    ...
    private void updateOtherBullets() {   // 상대방 총알 업데이트
    	 ...
    	 for (Iterator<Rectangle> it = otherBullets.iterator(); it.hasNext();) {
    	  ...
    	  for (Iterator<Rectangle> it = otherBulletsFlipped.iterator(); it.hasNext();) {
    	        ...
    	        if (myPlaneRect.intersects(bullet)) {
    	            ...
    	            it.remove(); // 충돌한 총알 제거
    	            boolean check = explosions.add(new Explosion(new Point(bullet.x-15, bullet.y))); // 폭발 추가
    	            if(check) {
    	            	myHP -= 5;  // HP 감소
    ...
    public void createAndShowGameFrame() {  // 게임 창 생성 및 위치 설정
        ...
        frame.setLocation(0, 0); // 화면 좌측 상단에 표시
        this.requestFocusInWindow();
    }
    
    private void checkGameOver() {  // 게임 종료 여부를 체크하는 함수
    	 if (myHP <= 0 || otherHP <= 0) {
    	        gameOver = true;
    ...
    public static void main(String[] args) {   	
    	Game_View gamePanel = new Game_View();   	
    }
}
